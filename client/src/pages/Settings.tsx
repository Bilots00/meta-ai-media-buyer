...
import { Settings as SettingsIcon, Save, Image as ImageIcon, X, LogOut } from "lucide-react";
...
interface Brand {
  ...
  topics: string; visuals: string[];
}
const empty: Brand = { name: "DreamBrothers", language: "Italiano", website: "", products: "", customers: "", location: "Worldwide", topics: "", visuals: [] };
...
export default function Settings() {
  ...
  const onFiles = async (files: FileList | null) => {
    ...
    } catch { toast.error("Errore nel caricamento immagini"); }
  };
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">My Brand — usato come contesto per la generazione dei contenuti</p>
          </div>
        </div>
      ...
      <div className="rounded-2xl p-6 space-y-5" style={{ background: "oklch(0.14 0.015 260)", border: "1px solid oklch(0.2 0.015 260)" }}>
        ...
        <div className="space-y-1.5">
          ...
          <Textarea rows={4} value={brand.products} onChange={e => set("products", e.target.value)} className="resize-none" style={inputStyle} placeholder="Descrivi il tuo brand, la sua missione, i valori..." />
        </div>
        <div className="grid sm:grid-cols-2 gap-4">
          <div className="space-y-1.5">
        ...
        <div className="space-y-1.5">
          <label className="text-sm text-muted-foreground">Di cosa parli? (temi)</label>
          <Textarea rows={3} value={brand.topics} onChange={e => set("topics", e.target.value)} className="resize-none" style={inputStyle} placeholder="Quali argomenti risuonano col tuo pubblico?" />
        </div>

        <div className="space-y-2">
          <label className="text-sm text-muted-foreground">Brand Visuals (opzionale)</label>
          <input id="brand-visuals" type="file" accept="image/*" multiple className="hidden" onChange={e => onFiles(e.target.files)} />
          <div className="flex gap-2 flex-wrap">
            ...
            <label htmlFor="brand-visuals" className="w-20 h-20 rounded-lg border border-dashed flex flex-col items-center justify-center cursor-pointer text-muted-foreground" style={{ borderColor: "oklch(0.25 0.02 260)" }}>
              ...
              <span className="text-[10px] mt-1">Allega</span>
            </label>
          </div>
        </div>
